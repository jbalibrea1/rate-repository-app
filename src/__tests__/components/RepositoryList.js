...
import { parseThousands } from '../../components/RepositoryItem.jsx';
  ...
  describe('RepositoryListContainer', () => {
    it('renders repository information correctly', () => {
      ...
      for (let i = 0; i < repositories.edges.length; i++) {
        ...
        expect(getAllByTestId('stargazers')[i]).toHaveTextContent(
          parseThousands(repositories.edges[i].node.stargazersCount)
        );
        expect(getAllByTestId('forks')[i]).toHaveTextContent(
          parseThousands(repositories.edges[i].node.forksCount)
        );
